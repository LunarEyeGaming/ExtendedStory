...
function init()
  ...
  self.monsterUniqueId = string.format("%s-voidghost", world.entityUniqueId(entity.id()) or sb.makeUuid())
...
function update(dt)
  ...
  erchiusCount = erchiusCount + (world.entityHasCountOfItem(entity.id(), "liquidsupervoid") or 0)
  ...
  if monsterPosition then
    if not self.messaged then
      world.sendEntityMessage(entity.id(), "queueRadioMessage", "supervoidsickness")
      self.messaged = true
    end
  ...
  elseif monsterPosition == nil then
    ...
    if self.spawnTimer < 0 then
      ...
      world.spawnMonster("supervoiddemon", vec2.add(mcontroller.position(), config.getParameter("ghostSpawnOffset")), parameters)
      self.spawnTimer = 1.0
    end
  end
