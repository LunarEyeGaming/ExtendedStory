...
function ChargeFire:fire()
  ...
  fireTimerZero = 0.01
  while self.fireMode == (self.activatingFireMode or self.abilitySlot) do
    fireTimerZero = math.max(0, fireTimerZero - self.dt)
	if fireTimerZero == 0 and status.overConsumeResource("energy", 10 * fireTimer * 5) then
	  fireTimerZero = 0.01
	  self:fireProjectile()
	  ...
	  util.wait(fireTimer, function()
        ...
        progress = math.min(1.0, progress + (self.dt / fireTimer))
      end)
	  self.weapon:setStance(self.stances.idle)
	end
