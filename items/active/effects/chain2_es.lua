      ...
      if segmentCount > 0 then
        ...
        for i = 1, lastDrawnSegment do
          ...
          if chain.waveform then
            local chainX = i * chain.segmentSize
            local chainXAnim = chainX - os.clock() * (chain.waveform.movement or 0)
            local segmentHeight = waveSegmentHeight(chainLength, chain.waveform.frequency, chain.waveform.amplitude, chainX, chainXAnim)
            thisOffset = vec2.add(thisOffset, {0, segmentHeight})
          end
...
function waveSegmentHeight(taperLength, period, amplitude, x1, x2)
