...
function destroy()
  if projectile.sourceEntity() and world.entityExists(projectile.sourceEntity()) then
    ...
    world.spawnProjectile("fusionbubble", mcontroller.position(), projectile.sourceEntity(), {math.cos(rotation), math.sin(rotation)}, false, { speed = 60, power = projectile.power(), powerMultiplier = projectile.powerMultiplier()})
