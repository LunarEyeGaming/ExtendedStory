ancientbossSwoopAttack = {}

function ancientbossSwoopAttack.enter()
  if not hasTarget() then return nil end
  if self.targetPosition == nil then return nil end

  return {
    timer = 0.0,
    swoopTime = config.getParameter("ancientbossSwoopAttack.swoopTime"),
    tookDamage = false
  }
end

function ancientbossSwoopAttack.enteringState(stateData)
  monster.setActiveSkillName("ancientbossSwoopAttack")
end

function ancientbossSwoopAttack.update(dt, stateData)
...
function ancientbossSwoopAttack.leavingState(stateData)
