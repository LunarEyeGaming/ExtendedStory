destroyerSwoopAttack = {}

function destroyerSwoopAttack.enter()
  if not hasTarget() then return nil end
  if self.targetPosition == nil then return nil end

  return {
    timer = 0.0,
    swoopTime = config.getParameter("destroyerSwoopAttack.swoopTime"),
    tookDamage = false
  }
end

function destroyerSwoopAttack.enteringState(stateData)
  monster.setActiveSkillName("destroyerSwoopAttack")
end

function destroyerSwoopAttack.update(dt, stateData)
...
function destroyerSwoopAttack.leavingState(stateData)
